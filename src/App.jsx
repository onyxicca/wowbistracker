...
import seasonItems from "./data/season_items_midnight_s1.json";
...
const WOW_CLASS_ICON_BASE = "https://wow.zamimg.com/images/wow/icons/large/";
...
const CLASSES = [
  {
    id:"death-knight", name:"Death Knight", color:"#C41E3A", classIcon:"classicon_deathknight",
  ...
  {
    id:"demon-hunter", name:"Demon Hunter", color:"#A330C9", classIcon:"classicon_demonhunter",
  ...
  {
    id:"druid", name:"Druid", color:"#FF7C0A", classIcon:"classicon_druid",
  ...
  {
    id:"evoker", name:"Evoker", color:"#33937F", classIcon:"classicon_evoker",
  ...
  {
    id:"hunter", name:"Hunter", color:"#AAD372", classIcon:"classicon_hunter",
  ...
  {
    id:"mage", name:"Mage", color:"#3FC7EB", classIcon:"classicon_mage",
  ...
  {
    id:"monk", name:"Monk", color:"#00FF98", classIcon:"classicon_monk",
    roles:["Tank","DPS","Healer"], armor:"Leather",
    weapons:["Main Hand + Off Hand"],
    specs:[
  ...
  {
    id:"paladin", name:"Paladin", color:"#F48CBA", classIcon:"classicon_paladin",
  ...
  {
    id:"priest", name:"Priest", color:"#E8E0D0", classIcon:"classicon_priest",
  ...
  {
    id:"rogue", name:"Rogue", color:"#FFF468", classIcon:"classicon_rogue",
  ...
  {
    id:"shaman", name:"Shaman", color:"#0070DD", classIcon:"classicon_shaman",
    roles:["DPS","Healer"], armor:"Mail",
    weapons:["2H Weapon","Main Hand + Off Hand"],
    specs:[
  ...
  {
    id:"warlock", name:"Warlock", color:"#8788EE", classIcon:"classicon_warlock",
    roles:["DPS"], armor:"Cloth",
    weapons:["Main Hand + Off Hand"],
    specs:[
  ...
  {
    id:"warrior", name:"Warrior", color:"#C69B3A", classIcon:"classicon_warrior",
    roles:["Tank","DPS"], armor:"Plate",
    weapons:["2H Weapon","Main Hand + Off Hand"],
    specs:[
...
.item-pool-pill{font-family:'Cinzel',serif;font-size:.55rem;letter-spacing:.06em;text-transform:uppercase;color:var(--gold-lt);border:1px solid rgba(201,146,42,.35);border-radius:999px;padding:.24rem .45rem;white-space:nowrap;background:rgba(201,146,42,.07)}
.item-pool-filters{grid-column:1 / span 2;display:flex;align-items:center;gap:.28rem;flex-wrap:wrap;margin-top:-.12rem}
.item-pool-filter-label{font-family:'Cinzel',serif;font-size:.52rem;letter-spacing:.08em;text-transform:uppercase;color:var(--parch-dk);opacity:.85;margin-right:.12rem}
.item-pool-filter{font-family:'Cinzel',serif;font-size:.52rem;letter-spacing:.05em;text-transform:uppercase;color:var(--parch-dk);border:1px solid rgba(201,146,42,.28);background:rgba(0,0,0,.12);border-radius:999px;padding:.18rem .42rem;cursor:pointer}
.item-pool-filter:hover{border-color:rgba(201,146,42,.5);color:var(--gold-lt)}
...
const SOURCE_TYPE_OPTIONS = ["", "Raid", "Dungeon", "Crafting", "Other"];
...
const RAID_DROP_SOURCE_OPTIONS = [
...
function sourceSummaryKind(sourceType) {
  if (sourceType === 'Raid') return 'Raid drop';
  if (sourceType === 'Dungeon') return 'Dungeon / M+';
  if (sourceType === 'Crafting') return 'Crafting';
  if (sourceType === 'Other') return 'Special / world';
...
function inferSourceType(src) {
  ...
  if (!s) return '';
  const raidHints = [
    'the singularity', 'chimaerus', 'vorasius', 'imperator averzian', 'midnight falls',
    'lightblinded vanguard', 'belo\'ren', 'beloren', 'vaelgor', 'crown of the cosmos',
    'march on quel\'danas', 'march on queldanas', 'voidspire', 'dreamrift'
  ];
  if (s.includes('raid') || CURRENT_SEASON_RAIDS.some(v => s.includes(v.toLowerCase())) || raidHints.some(v => s.includes(v))) return 'Raid';
  if (s.includes('dungeon') || s.includes('m+') || s.includes('mythic') || CURRENT_SEASON_DUNGEONS.some(v => s.includes(v.toLowerCase()))) return 'Dungeon';
  if (s.includes('craft') || s.includes('crafted') || s.includes('profession')) return 'Crafting';
  if (OTHER_SOURCE_OPTIONS.some(v => s.includes(v.toLowerCase())) || s.includes('delve') || s.includes('world') || s.includes('renown') || s.includes('ritual')) return 'Other';
  return '';
}
...
function sourceTypeFromBucket(bucket) {
  if (bucket === 'raid') return 'Raid';
  if (bucket === 'dungeon') return 'Dungeon';
  if (bucket === 'crafting') return 'Crafting';
  if (bucket === 'special' || bucket === 'pvp') return 'Other';
  return '';
}

function sourceDetailOptionsFor(type, currentDetail = '') {
  const base = getSpecificSourceOptions(type);
  const detail = String(currentDetail || '').trim();
  return detail && !base.includes(detail) ? [detail, ...base] : base;
}
...
const SEASON_ITEM_POOL = Array.isArray(seasonItems?.records) ? seasonItems.records.filter(item => item?.itemId && item?.itemName) : [];

const CLASS_ARMOR_TYPES = {
  "death knight": "plate",
...
const CLASS_WEAPON_TERMS = {
  "death knight": ["axe", "mace", "sword", "polearm"],
  "demon hunter": ["warglaive", "fist", "axe", "sword"],
  druid: ["dagger", "fist", "mace", "polearm", "staff", "off-hand", "held in off-hand", "holdable", "lantern"],
  evoker: ["dagger", "fist", "mace", "staff", "sword", "off-hand", "held in off-hand", "holdable"],
  hunter: ["axe", "bow", "crossbow", "dagger", "fist", "gun", "polearm", "staff", "sword"],
  mage: ["dagger", "staff", "sword", "wand", "off-hand", "held in off-hand", "holdable"],
  monk: ["axe", "fist", "mace", "polearm", "staff", "sword"],
  paladin: ["axe", "mace", "polearm", "shield", "sword"],
  priest: ["dagger", "mace", "staff", "wand", "off-hand", "held in off-hand", "holdable"],
  rogue: ["axe", "dagger", "fist", "mace", "sword"],
  shaman: ["axe", "dagger", "fist", "mace", "shield", "staff"],
  warlock: ["dagger", "staff", "sword", "wand", "off-hand", "held in off-hand", "holdable"],
  warrior: ["axe", "bow", "crossbow", "dagger", "fist", "gun", "mace", "polearm", "shield", "staff", "sword"],
};
...
const SPEC_PRIMARY_STAT = {
  druid: { balance: "intellect", restoration: "intellect", feral: "agility", guardian: "agility" },
  monk: { mistweaver: "intellect", brewmaster: "agility", windwalker: "agility" },
  paladin: { holy: "intellect", protection: "strength", retribution: "strength" },
  shaman: { elemental: "intellect", restoration: "intellect", enhancement: "agility" },
...
const ARMOR_FILTER_SLOTS = new Set(["head", "shoulders", "shoulder", "chest", "wrist", "hands", "waist", "legs", "feet"]);
const UNIVERSAL_FILTER_SLOTS = new Set(["neck", "back", "finger1", "finger2", "trinket1", "trinket2"]);
const WEAPON_FILTER_SLOTS = new Set(["weapon", "weapon2h", "mainhand", "offhand"]);
...
function classArmorType(value) {
  return CLASS_ARMOR_TYPES[normalizeClassName(value)] || "";
}

function specPrimaryStat(className, specName) {
  const classKey = normalizeClassName(className);
  const specKey = normalizeClassName(specName);
  return SPEC_PRIMARY_STAT[classKey]?.[specKey] || "";
}

function itemTierOwner(item) {
  const token = itemSlotToken(item);
  for (const [classKey, terms] of Object.entries(CLASS_TIER_TERMS)) {
    if (terms.some(term => token.includes(term))) return classKey;
  }
  return "";
}

function itemLooksLikeTier(item) {
  const token = itemSlotToken(item);
  const source = `${item?.sourceType || ""} ${item?.sourceLabel || ""}`.toLowerCase();
  return source.includes("tier") || source.includes("catalyst") || token.includes("set —") || !!itemTierOwner(item);
}

function itemPrimaryStats(item) {
  const direct = Array.isArray(item?.primaryStats) ? item.primaryStats : [];
  const statNames = Array.isArray(item?.statNames) ? item.statNames : [];
  const token = [...direct, ...statNames, item?.stats, item?.itemName]
    .flat()
    .map(value => String(value || "").toLowerCase())
    .join(" ");
  return ["strength", "agility", "intellect"].filter(stat => token.includes(stat));
}

function itemPoolMatchesClass(item, slotId, charClassName, charSpecName, broad = false) {
  if (broad) return true;
  const classKey = normalizeClassName(charClassName);
  if (!classKey || UNIVERSAL_FILTER_SLOTS.has(slotId)) return true;

  const token = itemSlotToken(item);
  const itemClass = String(item?.itemClass || "").toLowerCase();
  const itemSubclass = String(item?.itemSubclass || "").toLowerCase();

  if (itemLooksLikeTier(item)) {
    const owner = itemTierOwner(item);
    if (owner && owner !== classKey) return false;
    if (!owner && ARMOR_FILTER_SLOTS.has(slotId) && item?.sourceLabel && String(item.sourceLabel).toLowerCase().includes("tier")) return false;
  }

  if (ARMOR_FILTER_SLOTS.has(slotId)) {
    const armorType = CLASS_ARMOR_TYPES[classKey];
    if (!armorType) return true;
    const hasKnownArmorType = ["cloth", "leather", "mail", "plate"].some(type => itemSubclass.includes(type) || token.includes(type));
    if (!hasKnownArmorType) return true;
    return itemSubclass.includes(armorType) || token.includes(armorType);
  }

  if (WEAPON_FILTER_SLOTS.has(slotId)) {
    const allowed = CLASS_WEAPON_TERMS[classKey];
    if (!allowed) return true;
    if (slotId === "offhand" && (token.includes("shield") || itemSubclass.includes("shield"))) {
      return allowed.includes("shield");
    }
    if (itemClass.includes("armor") && (token.includes("off-hand") || token.includes("off hand") || token.includes("holdable") || token.includes("held in off-hand"))) {
      return allowed.some(term => token.includes(term));
    }
    if (!itemClass.includes("weapon") && !token.includes("weapon")) return false;
    if (!allowed.some(term => token.includes(term))) return false;
    const desiredStat = specPrimaryStat(charClassName, charSpecName);
    const stats = itemPrimaryStats(item);
    if (desiredStat && stats.length && !stats.includes(desiredStat)) return false;
    return true;
  }
...
function classFilterPill(charClassName, slotId, charSpecName, broad = false) {
  if (broad) return "Broad slot search";
  const armorType = classArmorType(charClassName);
  const stat = specPrimaryStat(charClassName, charSpecName);
  if (ARMOR_FILTER_SLOTS.has(slotId) && armorType) return `${armorType} + class filter`;
  if (WEAPON_FILTER_SLOTS.has(slotId) && charClassName) return stat ? `${charClassName} ${stat} weapons` : `${charClassName} weapons`;
  return "Strict pool";
}


const ITEM_SOURCE_FILTERS = [
  { value: "all", label: "All sources" },
  { value: "raid", label: "Raid" },
  { value: "dungeon", label: "Dungeon / M+" },
  { value: "crafting", label: "Crafting" },
  { value: "no-raid", label: "No raid" },
  { value: "special", label: "Special / world" },
];

const RAID_SOURCE_HINTS = [
  "lightblinded vanguard",
  "the singularity",
  "chimaerus",
  "vorasius",
...
function itemSourceBucket(item) {
  const raw = `${item?.sourceType || ""} ${item?.sourceLabel || ""} ${item?.sourceId || ""} ${item?.raidName || ""} ${item?.dungeonName || ""}`.toLowerCase();
  const sourceType = String(item?.sourceType || "").toLowerCase();
  if (sourceType === "raid") return "raid";
  if (sourceType === "dungeon") return "dungeon";
  if (sourceType === "crafting" || sourceType === "crafted") return "crafting";
  if (sourceType === "other" && (raw.includes("tier set") || raw.includes("catalyst") || raw.includes("vault"))) return "raid";
  if (raw.includes("raid") || CURRENT_SEASON_RAIDS.some(v => raw.includes(v.toLowerCase())) || RAID_SOURCE_HINTS.some(v => raw.includes(v))) return "raid";
  if (raw.includes("dungeon") || raw.includes("mythic") || raw.includes("m+") || CURRENT_SEASON_DUNGEONS.some(v => raw.includes(v.toLowerCase()))) return "dungeon";
  if (raw.includes("craft") || raw.includes("profession")) return "crafting";
  if (raw.includes("pvp")) return "pvp";
  if (raw.includes("delve") || raw.includes("world") || raw.includes("renown") || raw.includes("prey") || raw.includes("ritual")) return "special";
  return "unknown";
}

function itemMatchesSourceFilter(item, sourceFilter = "all") {
  const bucket = itemSourceBucket(item);
  if (!sourceFilter || sourceFilter === "all") return true;
  if (sourceFilter === "raid") return bucket === "raid";
  if (sourceFilter === "dungeon") return bucket === "dungeon";
  if (sourceFilter === "crafting") return bucket === "crafting";
  if (sourceFilter === "no-raid") return bucket !== "raid" && bucket !== "unknown";
  if (sourceFilter === "special") return bucket === "special" || bucket === "pvp";
  return true;
}

function manualPlanSourceSummary(data = {}) {
  const items = [];
  Object.values(data || {}).forEach(slot => {
    if (!slot || !Array.isArray(slot.ranks)) return;
    slot.ranks.forEach((rank, idx) => {
      if (!rank?.name) return;
      const src = rank.sourceType || inferSourceType(rank.src || rank.sourceLabel || "") || "Unknown";
      const bucket = itemSourceBucket({ sourceType: src, sourceLabel: rank.src || rank.sourceLabel || "" });
      items.push({ idx, name: rank.name, src, bucket, status: idx === 0 ? "best" : (rank.status || DEFAULT_MANUAL_STATUS[idx] || "alt") });
    });
  });
  const counts = items.reduce((acc, item) => {
    acc.total += 1;
    acc[item.bucket] = (acc[item.bucket] || 0) + 1;
    if (item.bucket === "raid" && (item.idx === 0 || item.status === "best" || item.status === "equivalent")) acc.raidCore += 1;
    return acc;
  }, { total: 0, raid: 0, dungeon: 0, crafting: 0, special: 0, pvp: 0, unknown: 0, raidCore: 0 });
  const level = counts.raidCore >= 4 || counts.raid >= 6 ? "Severe" : counts.raidCore >= 2 || counts.raid >= 3 ? "High" : counts.raidCore >= 1 || counts.raid >= 1 ? "Medium" : counts.total ? "Low" : "Not calculated";
  return { items, counts, level };
}

function ManualSourceSummary({ data }) {
  const summary = manualPlanSourceSummary(data);
  if (!summary.counts.total) return null;
  const chips = [
    ["Raid", summary.counts.raid],
    ["Dungeon / M+", summary.counts.dungeon],
    ["Crafting", summary.counts.crafting],
    ["Special", summary.counts.special],
    ["Unknown", summary.counts.unknown],
  ].filter(([, count]) => count);
  return (
    <div className="no-print" style={{ marginTop: ".65rem", padding: ".65rem .75rem", border: "1px solid var(--bdr)", background: "rgba(0,0,0,.16)", width: "100%" }}>
      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", gap:".45rem", marginBottom:".35rem" }}>
        <span style={{ fontFamily:"Cinzel,serif", fontSize:".68rem", letterSpacing:".1em", color:"var(--gold)" }}>Raid Dependency: {summary.level}</span>
        <span style={{ fontSize:".72rem", color:"var(--parch-dk)" }}>{summary.counts.raidCore} core/equivalent raid target{summary.counts.raidCore === 1 ? "" : "s"}</span>
      </div>
      <div style={{ display:"flex", flexWrap:"wrap", gap:".35rem" }}>
        {chips.map(([label, count]) => (
          <span key={label} style={{ fontFamily:"Cinzel,serif", fontSize:".6rem", letterSpacing:".06em", border:"1px solid var(--bdr2)", color:"var(--parch-dk)", padding:".12rem .42rem", background:"rgba(255,255,255,.03)" }}>{label}: {count}</span>
        ))}
      </div>
      <div style={{ fontSize:".72rem", color:"var(--parch-dk)", marginTop:".4rem", fontStyle:"italic", lineHeight:1.45 }}>
        This is source transparency only. It shows where this Manual plan asks you to farm; it does not estimate the performance gap until sim-informed values exist.
      </div>
    </div>
  );
}

function itemSlotToken(item) {
  return [item?.slot, item?.inventoryType, item?.itemClass, item?.itemSubclass, item?.itemName]
    .map(value => String(value || "").toLowerCase())
    .join(" ");
}
...
const SLOT_MATCHERS = {
  ...
  back: ["back", "cloak"],
  chest: ["chest", "robe"],
  wrist: ["wrist", "bracer", "bracers", "wristguard", "wristguards", "cuff", "cuffs"],
  hands: ["hand", "hands", "glove", "gloves", "gauntlet", "gauntlets", "grip", "grips"],
  waist: ["waist", "belt", "cinch", "girdle", "sash"],
  legs: ["leg", "legs", "legging", "leggings", "breeches", "pants", "wraps"],
  feet: ["feet", "foot", "boots", "boot", "treads", "sandals", "walkers"],
  finger1: ["finger", "ring", "band"],
  finger2: ["finger", "ring", "band"],
  trinket1: ["trinket"],
  trinket2: ["trinket"],
};

function itemPoolMatchesSlot(item, slotId) {
  if (!slotId) return true;
  const token = itemSlotToken(item);
  const inv = String(item?.slot || item?.inventoryType || "").toLowerCase();
  const cls = String(item?.itemClass || "").toLowerCase();
  const subclass = String(item?.itemSubclass || "").toLowerCase();

  if (slotId === "weapon2h") {
    return cls.includes("weapon") && (
      inv.includes("two-hand") ||
      inv.includes("twoh") ||
      token.includes("two-hand") ||
      token.includes("staff") ||
      token.includes("polearm") ||
      token.includes("greatsword") ||
      token.includes("two-handed")
    );
  }

  if (slotId === "mainhand") {
    return cls.includes("weapon") && !inv.includes("off hand") && !inv.includes("off-hand") && !inv.includes("holdable") && !inv.includes("shield");
  }

  if (slotId === "weapon") {
    return cls.includes("weapon") || token.includes("weapon");
  }

  if (slotId === "offhand") {
    return (
      inv.includes("off hand") ||
      inv.includes("off-hand") ||
      inv.includes("held") ||
      inv.includes("holdable") ||
      inv.includes("shield") ||
      subclass.includes("shield") ||
      token.includes("off hand") ||
      token.includes("off-hand") ||
      token.includes("lantern")
    );
  }

  const matchers = SLOT_MATCHERS[slotId] || [slotId];
  return matchers.some(part => token.includes(part));
}

function seasonItemOptionsForSlot(slotId, charClassName, charSpecName, broad = false, sourceFilter = "all") {
  return SEASON_ITEM_POOL
    .filter(item => itemPoolMatchesSlot(item, slotId) && itemPoolMatchesClass(item, slotId, charClassName, charSpecName, broad) && itemMatchesSourceFilter(item, sourceFilter))
...
function findSeasonItem(value, slotId, charClassName, charSpecName, broad = false, sourceFilter = "all") {
  ...
  const search = list => list.find(item => {
    ...
    if (seasonItemOptionLabel(item).toLowerCase() === lower) return true;
    return String(item.itemName || "").toLowerCase() === lower;
  }) || null;
...
function ItemPoolPicker({ id, idx, value, onPick, onSearch, onUnknownItem, charClassName, charSpecName }) {
  ...
  const options = seasonItemOptionsForSlot(id, charClassName, charSpecName, broad, sourceFilter);
  const slotLabel = SLOT_LABELS[id] || "this slot";
  return (
    <div className="item-pool-picker no-print">
      ...
      />
      <button type="button" className="item-pool-pill" onClick={() => setBroad(v => !v)} title={broad ? "Return to strict class/spec filtering" : "Show broader slot matches if an item is being hidden"}>
        {options.length ? classFilterPill(charClassName, id, charSpecName, broad) : broad ? "No broad matches" : "No strict matches"}
      </button>
      <div className="item-pool-filters" aria-label="Suggestion source filters">
        <span className="item-pool-filter-label">Suggestions:</span>
        {ITEM_SOURCE_FILTERS.map(opt => (
          <button
            key={opt.value}
            type="button"
            className={"item-pool-filter" + (sourceFilter === opt.value ? " active" : "")}
            onClick={() => setSourceFilter(opt.value)}
            title="Filter the suggestion list without changing saved item details"
          >
            {opt.label}
          </button>
        ))}
      ...
      <datalist id={listId}>
        {options.map(item => <option key={item.itemId} value={seasonItemOptionLabel(item)} />)}
      </datalist>
    </div>
  );
}
...
function Slot({ label, id, data, onChange, targetTrack, bisMode, equipped, charClassName, charSpecName, printTemplate = "screen" }) {
  ...
  if (bisMode === "custom") {
    ...
    const upRank = (idx, field, val) => patchRank(idx, { [field]: val });
    const applySeasonItem = (idx, item) => {
      if (!item) return;
...
function Tracker({ cls, spec, charName, initialMode = "", onBack }) {
  ...
  const slotViewGroups = [
    { id: "all", label: "All", slots: [] },
    { id: "head", label: "Head", slots: ["head"] },
    { id: "neck", label: "Neck", slots: ["neck"] },
    { id: "shoulders", label: "Shoulders", slots: ["shoulders", "shoulder"] },
    { id: "chest", label: "Chest", slots: ["chest"] },
    { id: "wrist", label: "Wrists", slots: ["wrist"] },
    { id: "hands", label: "Hands", slots: ["hands"] },
    { id: "waist", label: "Belt", slots: ["waist"] },
    { id: "legs", label: "Pants", slots: ["legs"] },
    { id: "feet", label: "Boots", slots: ["feet"] },
    { id: "rings", label: "Rings", slots: ["finger1", "finger2"] },
    { id: "trinkets", label: "Trinkets", slots: ["trinket1", "trinket2"] },
    { id: "weapons", label: "Weapons", slots: ["weapon2h", "mainhand", "offhand"] },
  ];
  const slotViewMatches = (slotId) => {
    if (slotView === "all") return true;
    const group = slotViewGroups.find(v => v.id === slotView);
    return group ? group.slots.includes(slotId) : true;
  };
  const sectionVisible = (...slotIds) => slotView === "all" || slotIds.some(slotViewMatches);
...
🎁 Great Vault: ${vaultHits.length} target item${vaultHits.length !== 1 ? "s" : ""} found in your vault this week.` : "";
      ...
      <div className="slot-jump-row no-print" style={{ display:"flex", flexWrap:"wrap", gap:".35rem", alignItems:"center", margin:".75rem 0 .95rem", padding:".55rem .65rem", border:"1px solid var(--bdr)", background:"rgba(0,0,0,.16)" }}>
        {slotViewGroups.map(group => (
          <button
            key={group.id}
            type="button"
            onClick={() => setSlotView(group.id)}
            style={{
              ...
              padding:".25rem .55rem",
              border:`1px solid ${slotView === group.id ? "var(--gold)" : "var(--bdr2)"}`,
              background: slotView === group.id ? "rgba(201,146,42,.22)" : "rgba(0,0,0,.22)",
              color: slotView === group.id ? "var(--gold-lt)" : "var(--parch-dk)",
              cursor:"pointer"
            }}
          >
            {group.label}
      ...
      {sectionVisible(...LEFT_SLOTS.map(s => s.id), ...RIGHT_SLOTS.map(s => s.id)) && (
        <div className="gear-grid">
          {LEFT_SLOTS.map((ls, i) => {
            const rs = RIGHT_SLOTS[i];
            const showLeft = slotViewMatches(ls.id);
            const showRight = rs && slotViewMatches(rs.id);
      ...
      <div className="class-grid">
        {filtered.map(cls => (
          <div key={cls.id} className="cc" style={{ "--cc-color": cls.color }} onClick={() => onSelectClass(cls)}>
            {cls.classIcon && <img className="cc-bg-icon" src={`${WOW_CLASS_ICON_BASE}${cls.classIcon}.jpg`} alt="" aria-hidden="true" loading="lazy" />}
